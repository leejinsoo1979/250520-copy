...
const StepViewTwoDViewer = ({ 
  ...
  // Enforce AC soffit depth always equals wall depth
  const resolvedAcUnitDepth = dimensions.depth;
  // Soffit pillar height/thickness always uses acUnitDepth
  ...
  const drawTopView = (ctx) => {
    ...
    const innerFrameOffsetPx = 580 * scale;                  // back wall으로부터 580mm
    ctx.beginPath();
    ctx.rect(x, y + innerFrameOffsetPx, widthPx, innerFrameThicknessPx);
    ctx.fill();
    ctx.strokeStyle = outlineColor;
    ctx.lineWidth = 1;
    ctx.stroke();
  ...
  const drawFrontView = (ctx) => {
    ...
    // 프레임 두께(벽체프레임) 예시값, 실제 값은 옵션에서 받아올 수 있음
    const leftFrameWidth = options?.leftFrameWidth ?? FRAME_DIMENSIONS.default.leftFrameWidth;   // 50mm
    const rightFrameWidth = options?.rightFrameWidth ?? FRAME_DIMENSIONS.default.rightFrameWidth; // 50mm
    const upperFrameWidth = options?.upperFrameWidth ?? FRAME_DIMENSIONS.default.upperFrameWidth; // 40mm
    ...
    if ((options?.hasBase || hasBaseFromOptions) && baseHeight > 0) {
      ...
      const baseDepth = options?.baseDepth || 580; // 기본 깊이는 580mm
      const baseInsideFrame = true; // 항상 프레임 내부에 위치하도록 수정
      ...
      // 항상 프레임 내부에 위치하도록 설정
      const baseX = x + leftFrameWidth * scale;
      const baseWidthPx = widthPx - (leftFrameWidth + rightFrameWidth) * scale;
      
      // 받침대 채우기 - 포커스에 따라 색상 변경
      ctx.fillStyle = activeDimensionField === 'baseHeight' ? '#FF69B4' : frameColor; // 포커스 시 분홍색, 아니면 프레임과 동일 색상
      ctx.fillRect(baseX, baseY, baseWidthPx, baseHeightPx);
      
      // 받침대 테두리
      ctx.strokeStyle = '#999999';
      ctx.lineWidth = 1;
      ctx.strokeRect(baseX, baseY, baseWidthPx, baseHeightPx);
      
      // 받침대 깊이 표시 (앞쪽 라인) - Step 3에서는 깊이(Z축) 관련 정보 숨김
      if (baseDepth < dimensions.depth && !isStep3) {
        // 앞쪽 경계 점선
        ctx.beginPath();
        ctx.setLineDash([4, 4]);
        ctx.moveTo(baseX, baseY + baseHeightPx - 1);
        ctx.lineTo(baseX + baseWidthPx, baseY + baseHeightPx - 1);
        ctx.stroke();
        ctx.setLineDash([]);
      }
    }
  ...
  const drawLeftView = (ctx) => {
    ...
    const dividerThicknessPx = 20 * scale;
    // built-in depth offset from back wall (right side)
    const dividerX = x + depthPx - acUnitWidth * scale - dividerThicknessPx;
    ctx.fillStyle = frameColor;
    ctx.beginPath();
    ctx.rect(
      dividerX,
  ...
  const drawDimensionLines = (ctx, viewType, x, y, widthPx, heightPx, depthPx, scale) => {
    ...
    // 프레임 치수선 그리기 함수
    const drawFrameDimension = (startX, startY, endX, endY, value, frameType, isVertical) => {
      ...
      drawDimensionWithExtension(
        ...
        { noStacking: true, color: dimensionColor, frameType }
      );
    };
      ...
      if (isVertical) {
        ...
        if (typeof widthPx !== 'undefined' && widthPx > 0) {
          ...
          if (isLeft) {
            ...
            dimensionX = x + depthPx + gapFromObject; // 오른쪽 간격 유지
          }
        } else {
          // fallback
          dimensionX = startX - gapFromObject; // 기본 간격 유지
        }
        
        // 모든 치수 타입(프레임 포함)에 대해 동일한 extensionLineGap 사용
        const startOffsetX = isLeft ? -extensionLineGap : extensionLineGap;
        
        // 객체와 보조선 사이 간격 추가 (Step 3에서 보조선이 객체에 붙지 않도록)
        const objectGap = 5;
        const objGapX = isLeft ? -objectGap : objectGap;
        
        ctx.moveTo(startX + objGapX, startY);
        ctx.lineTo(dimensionX, startY);
        ctx.moveTo(endX + objGapX, endY);
        ctx.lineTo(dimensionX, endY);
        ctx.moveTo(dimensionX, startY);
        ctx.lineTo(dimensionX, endY);
        ctx.stroke();
        drawSlashEnd(dimensionX, startY, true, isVertical, dimensionColor);
        drawSlashEnd(dimensionX, endY, false, isVertical, dimensionColor);
        const midY = (startY + endY) / 2;
        const textWidth = ctx.measureText(`${measurementText}`).width;
        ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
        ctx.fillRect(
          dimensionX - textWidth/2 - 6, 
          midY - 12,
          textWidth + 12,
          24
        );
        ctx.fillStyle = dimensionColor;
        ctx.fillText(`${measurementText}`, dimensionX, midY);
      } else {
        // Horizontal (수평선): always use fixed offset above or below geometry, no stacking
        let dimensionY;
        // For horizontal lines: if above geometry, dimensionY = y - gapFromObject
        // If below geometry, dimensionY = y + heightPx + gapFromObject (or y + depthPx + gapFromObject for top view)
        // For front/top: heightPx/depthPx
        let isAbove;
        if (typeof heightPx !== 'undefined' && heightPx > 0) {
          isAbove = (startY <= y + heightPx / 2);
          if (isAbove) {
            dimensionY = y - gapFromObject; // 상단 간격 유지 (30px로 일관되게 적용)
          } else {
            dimensionY = y + heightPx + gapFromObject; // 하단 간격 유지 (30px로 일관되게 적용)
          }
        } else if (typeof depthPx !== 'undefined' && depthPx > 0) {
    ...
    if (viewType === 'front') {
      ...
      if (options?.step === 'step3') {
        ...
        */
        ...
        // 단내림이 있는 경우
        if (hasAirConditioner) {
          // 1. 에어컨 단내림 높이 치수는 표시하지 않음
          /* 단내림 높이 치수 표시 제거
          drawDimensionWithExtension(
            segmentedSideX, y,
            segmentedSideX, y + acHeightPx,
            true,
            `${resolvedAcUnitHeight}`,
            1,
            { noStacking: true }
          );
          */
    ...
    if (viewType === 'front' && options?.step === 'step3') { // 스텝3에서만 표시되도록 조건 사용
      ...
      // 세미스탠딩 모드에서의 벽 위치 결정
      const isLeftWall = spaceType === 'semi-standing' && position === 'left';
      const isRightWall = spaceType === 'semi-standing' && position === 'right';
...
export default StepViewTwoDViewer;